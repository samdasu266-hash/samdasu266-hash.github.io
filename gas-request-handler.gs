 ...
 * 기관 ID → 표시 이름.
 *
 * 사이트가 발행하는 institutions.json 에서 매 실행 시 읽어온다.
 * 기관이 추가돼도 이 파일을 고치거나 재배포할 필요가 없다.
 * (build_institution_pages.py 의 INSTITUTIONS 한 곳만 고치면 반영된다)
 *
 * 아래 목록은 institutions.json 을 못 읽었을 때만 쓰는 예비값이다.
 * 예비값에 없는 ID 는 ID 자체를 그대로 표시한다.
 */
var INST_NAMES_FALLBACK = {
...
var _instNames = null;  // 실행 중 1회만 조회

function instNames_() {
  if (_instNames) return _instNames;
  try {
    var res = UrlFetchApp.fetch(INSTITUTIONS_URL, { muteHttpExceptions: true });
    if (res.getResponseCode() === 200) {
      var list = JSON.parse(res.getContentText()).institutions || [];
      var map = {};
      for (var i = 0; i < list.length; i++) {
        if (list[i] && list[i].id) map[list[i].id] = list[i].name || list[i].id;
      }
      if (Object.keys(map).length) { _instNames = map; return _instNames; }
    }
    Logger.log("institutions.json 조회 실패 — 예비 목록 사용");
  } catch (e) {
    Logger.log("institutions.json 조회 오류: " + e + " — 예비 목록 사용");
  }
...
function instName_(id) {
  return instNames_()[id] || id;
}
